...
sys.path.append('/home/jay/t-prime')
...
def process_all_packets(model, class_map, seq_len, sli_len, channel, protocol, test_data_path, output_base_path):
    ...
    for idx, signal_path in enumerate(mat_list):
        ...
        #idxs = list(range(seq_len * sli_len * 2, 2 * len_sig, seq_len * sli_len * 2))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', default='DATASET1_1_TEST/1', help='Path to test dataset folder')
    ...
    model = AMC_Net(num_classes=len(PROTOCOLS))
    model.load_state_dict(torch.load(f"/home/jay/t-prime-STABLE-2/baseline_models/AMC_net_weights/model.AMCNet.random.range.pt", map_location=device)['model_state_dict'])
    model.to(device)
    model.eval()
