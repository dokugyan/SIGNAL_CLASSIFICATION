...
if __name__ == "__main__":
    ...
    model = TransformerModel(classes=len(PROTOCOLS), d_model=64 * 2, seq_len=24, nlayers=2, use_pos=False)
    model.load_state_dict(torch.load(f"{TRANS_PATH}/modelrandom_range_sm.pt", map_location=device)['model_state_dict'])
    model.to(device)
    model.eval()

    for protocol in args.protocols:
        for channel in CHANNELS:
            process_all_packets(model, class_map, seq_len=24, sli_len=64, channel=channel,
